...
procedure exercise7 is
    ...
    begin
        ...
        loop		-- Infinite loop, showing the current round what it is working
            ...
            begin			-- Each worker try to increment a variable (which can randomly activate an exception)
                Num := Unreliable_Slow_Add(Prev);
            exception		-- Only when error "Count_Failed" is detected ("catch" in C/Java), abort signal is sent
                when Count_Failed =>
                    Manager.Signal_Abort;	-- Worker informs the Manager that process is aborted
            end;
            Manager.Finished;				-- Worker informs the Manager that he has finished
            -------------------------------------------
            ...
            if Manager.Commit = True then
                ...
                Num := Prev;	-- In case Commit = False (at least one Worker could not do the job), all Workers will recover the previous value
                -------------------------------------------
            end if;
